...
{
    public Cia2(CPU cpu, ILoggerFactory loggerFactory)
        : base(cpu, new CiaIRQ(useNMI: true)) // Initialize CIA #1 timer with to raise NMI instead of IRQ
    {
    }
    ...
    public byte DataALoad(ushort address)
    {
        ...
        var value = ReadIOStorage(address);
